...
gri   = (
            k = exp.(collect(range(xxx,xxx;length = mpar.nk))) .- xxx .+xxx,
            z = [1/9; 10/9]
        )
...
while distVF[iterVF]>mpar.crit # Value Function iteration loop: until distance is smaller than crit.
    ...
    global Vnew,kprime = VFI_update_spline(xxx,xxx,xxx,par,mpar,gri,Π) # Optimize given cont' value
...
C       = (xxx  + xxx) #Initial guess for consumption policy: roll over assets
Cold    = C # Save old policy
distEG  = ones(1) # Initialize Distance
iterEG  = 1 # Initialize Iteration count
while distEG[iterEG]>mpar.crit
    global C,    = EGM(xxx, mutil,invmutil,par,mpar,Π,meshes,gri) # Update consumption policy by EGM
    local dd     = maximum(abs.(xxx-xxx)) # Calculate Distance

    global Cold   = C # Replace old policy
    global iterEG += 1 #count iterations
    append!(distEG,dd)
end
C,Kprimestar = EGM(xxx,xxx,xxx,par,mpar,Π,meshes,gri)
